...
class SubmissionOutcome(Enum):
    ...
    See routes.py::simpleMNOVA_display_molecule for the source of truth:
      - SUCCESS: status 200, non-JSON body -> the rendered D3 HTML page.
      ...
      - ERROR: any non-200 status (400 validation errors, or an arbitrary
        e.code from PipelineError which is not guaranteed to be a
        conventional HTTP status), or a 200 JSON body that doesn't match
        the known {"status": "unregistered"/"registration_expired"} shape.
    ...
    def make_request():
        try:
            ...
            if is_json:
                ...
                # Real success: the rendered D3 HTML page.
                workingFilename = json_data["workingFilename"]["data"].get(
                    "0", "nmr_analysis_result"
                )
                response_text = response.text.replace("dummy_title", workingFilename)
                ...
                # Non-200: 400 validation errors, or an arbitrary
                # PipelineError code/message. Never write/open this as
                # if it were the result page.
