                            ...
                            try:
                                ...
                                if bond_pair not in bonds_added:
                                    ...
                                    if bond_order == 1:
                                        ...
                                        bond_name = "TRIPLE"
                                    elif bond_order in (513, 770, 769):  # Aromatic
...
class jeolData:
    def __init__(self, file_path: Path):
        ...
        print("Adding spec_info to data with peaks")
        for expt_id, data in self.datasets_with_peaks.items():
            ...
            print(data["spec_info"])

        # add peakList to data with peaks
        print("\nAdding peakList to data with peaks\n")
        for expt_id, data in self.datasets_with_peaks.items():
            print("expt_id:", expt_id)
            data["peaks"]["data"] = {}
            data["peaks"]["count"] = 0
    ...
    def append_specdata(self):
        ...
        for expt_ky, expt_type in zip(expt_kys, expt_types):
            # print(f"Looking for experiment: {expt_fn}, Type: {expt_type}")
            data = None
            # for id, dataset in self.datasets_with_peaks.items():
            #     if dataset["spec_info"]["expt_fn"] == expt_fn:
            #         expt_ky = id
            #         data = dataset
            #         break
            data = self.datasets_with_peaks.get(expt_ky, None)
            print(f"Experiment: {expt_ky}, Type: {expt_type}")
            if data:
...
if __name__ == "__main__":
    ...
    json_file_path = Path(jeol_fn.parent,  jeol_fn.name.replace(jeol_fn.suffix, "_jeol_inputdata.json"))
